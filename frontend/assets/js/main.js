...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loginForm && loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (typeof window.showToast === 'function') {
                window.showToast('تم تسجيل الدخول بنجاح! جاري التحويل...', 'success');
            }
            ...
            const message = (err && err.detail) ? err.detail : 'فشل تسجيل الدخول، الرجاء المحاولة مرة أخرى';
            if (typeof window.showToast === 'function') {
    ...
    registerForm && registerForm.addEventListener('submit', async (e) => {
        ...
        const nationalId = document.getElementById('regNationalId') ? document.getElementById('regNationalId').value.trim() : '';
        const governorate = document.getElementById('regGovernorate') ? document.getElementById('regGovernorate').value : '';
        ...
        if (pass !== passConfirm) {
            if (typeof window.showToast === 'function') {
                window.showToast('كلمتا المرور غير متطابقتين. الرجاء التحقق وإعادة المحاولة.', 'warning');
            } else {
        ...
        try {
            ...
            // Show success toast
            if (typeof window.showToast === 'function') {
                window.showToast('تم إنشاء الحساب بنجاح! يرجى التحقق من بريدك الإلكتروني لتفعيل الحساب.', 'success');
            }
            ...
            const message = (err && err.detail) ? err.detail : 'فشل إنشاء الحساب، الرجاء المحاولة مرة أخرى';
            if (typeof window.showToast === 'function') {
                window.showToast(message, 'error');
            } else {
                alert(message);
